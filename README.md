...
- 공개 검증 허브: Trust Center, 시장 리포트, 공유 조회, 피드백, CTA 실험, 공개 유입 표면을 proof 카드로 집계
- SQLite 기반 분석 결과 저장
- 저장 리포트 조회와 가격 알림 구독
...
- `/public/proof-hub`: Trust Center, 공개 시장 리포트, 공유 조회, 피드백, 출시 CTA 실험, 공개 유입 표면을 공개 proof 카드와 반박 답변으로 집계
